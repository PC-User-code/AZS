...
def index():
    ...
    return render_template("index.html", title="Главная", base_data=base_data,
                           today=today, dates=[i for i in range(24)], data=[1, 2, 5, 6, 4, 3, 7, 8, 9, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2], news=news,
                           temp=temp,
...
def render_graphics():
    ...
    return render_template("render_graphics.html", date=date, base_data=base_data,
                           ...
                           data=[1,2,5,6,4,3,7,8,9,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2], table_data=table_data, title="Статистика")

@app.route("/today")
def day_statistic():
    statistic = get_data("today")
    return render_template("render_graphics.html", date=statistic[0], today=today, title="Статистика за день",
...
def week_statistic():
    ...
    return render_template("render_graphics.html", date=statistic[0], today=today, title="Статистика за неделю",
                           dates=[i for i in range(24)], base_data=base_data,
                           data=[1,2,5,6,4,3,7,8,9,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2])
...
def month_statistic():
    ...
    return render_template("render_graphics.html", date=statistic[0], today=today, title="Статистика за месяц",
                           dates=[i for i in range(24)], base_data=base_data,
                           data=[1,2,5,6,4,3,7,8,9,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2])
@app.route("/tg_bot")
def tg_bot():
...
if __name__ == '__main__':
    ...
    base_data = {"address": "Татарская, 69", "date": "-".join(str(datetime.datetime.today()).split()[0].split("-")[::-1])}
    main()
    db_sess = db_session.create_session()
